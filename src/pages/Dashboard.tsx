...
import { CompareChart, CompareRow, StatTile } from '../components/ui/Metrics';
import { Badge } from '../components/ui/Badge';
import { ScoreRing } from '../components/ui/ScoreRing';
import { formatHours, formatPct, formatRs, formatRsShort } from '../utils/format';

export function Dashboard() {
  const { result, preferences } = usePlanner();

  if (!result) return <EmptyPipelineState moduleName="Dashboard" />;

  const selected = result.plans.find((p) => p.id === result.recommendedPlanId) ?? null;
  const feasible = result.plans.filter((p) => !p.excluded);
